  ...
  <div class="w-1/4 min-w-[280px] max-w-[400px] flex-shrink-0 flex flex-col bg-gray-50/50 rounded-2xl border border-gray-200/50 overflow-hidden backdrop-blur-sm">
    ...
    <div v-else-if="projects.length > 0" class="flex-1 overflow-y-auto px-3 pb-3 space-y-1 custom-scrollbar">
      <div v-for="project in projects" :key="project.id" class="space-y-2">
        ...
        <div
          v-if="getProjectSchedules(project.id, upcomingSchedules).length > 0"
          class="ml-6 pl-3 border-l border-gray-200 my-1 space-y-0.5"
        >
          <div
            v-for="schedule in getProjectSchedules(project.id, upcomingSchedules).slice(0, 3)"
            :key="schedule.id"
            ...
            @click.stop="emit('toggle-schedule', schedule)"
          >
            <div
              class="w-1.5 h-1.5 rounded-full flex-shrink-0 transition-transform group-hover/schedule:scale-125"
              :class="isScheduleSelected(schedule.id, selectedSchedules) ? 'bg-blue-500' : 'bg-gray-400'"
            ></div>
          ...
          <div
            v-if="getProjectSchedules(project.id, upcomingSchedules).length > 3"

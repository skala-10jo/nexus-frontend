<template>
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <button v-for="(session, idx) in sessions" :key="idx" @click="$emit('start-session', idx)"
        class="p-5 rounded-xl border-2 transition-all duration-200 text-left" :class="session.completed
          ? 'bg-green-50 border-green-500'
          : 'bg-white border-gray-200 hover:border-gray-400 hover:shadow-sm'">
        <div class="flex justify-between items-start mb-3">
          <div class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
            <span class="flex items-center justify-center w-10 h-10 rounded-full font-bold mb-2 md:mb-0"
        ...
        <div class="space-y-1 text-center md:text-left">
          <p class="text-xs text-gray-500 truncate" v-for="(expr, exprIdx) in session.expressions.slice(0, 3)"
            :key="exprIdx">
            • {{ expr.expression }}
          </p>
        ...
        <div class="mt-4 pt-3 border-t border-gray-100 text-center md:text-left">
          <span class="text-sm font-medium" :class="session.completed ? 'text-green-600' : 'text-gray-700'">
